...
define( 'CUSTOM_METABOXES_DIR', get_template_directory_uri() . '/plugins/metaboxes' );
...
function custom_meta_box_field( $field, $meta = null, $repeatable = null ) {
	...
	switch( $type ) {
		...
		case 'post_checkboxes':
			$posts = get_posts( array( 'post_type' => $post_type, 'posts_per_page' => -1, 'suppress_filters' => 0 ) );
			echo '<ul class="meta_box_items">';
			foreach ( $posts as $item ) 
		...
		case 'post_drop_sort':
			...
			$posts = get_posts( array( 'post_type' => $post_type, 'posts_per_page' => -1, 'post__not_in' => $exclude, 'suppress_filters' => 0 ) );
		...
		case 'tax_select':
			...
			$terms = get_terms( $tax_id, 'get=all&suppress_filters=0' );
			...
			echo '</select> &nbsp;<span class="description"><a href="'.home_url() . '/wp-admin/edit-tags.php?taxonomy=' . $tax_id . '">Manage ' . $taxonomy->label . '</a></span>
				<br />' . $desc;
		break;
		// tax_checkboxes
		case 'tax_checkboxes':
			...
			echo '<span class="description">' . (isset($field['desc']) ? $field['desc'] : '') . ' <a href="'. home_url() . '/wp-admin/edit-tags.php?taxonomy=' . $tax_id . '">Manage ' . $taxonomy->label . '</a></span>';
		break;
		// date
		case 'date':
