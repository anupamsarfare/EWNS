...
function accommodation_vacancy_admin_page() {
	$hook = add_submenu_page('edit.php?post_type=accommodation', __('BYT Accommodation vacancy management', 'bookyourtravel'), __('Vacancies', 'bookyourtravel'), 'edit_posts', basename(__FILE__), 'accommodation_vacancies_admin_display');
...
add_action( 'wp_ajax_admin_accommodation_is_self_catered_ajax_request', 'admin_accommodation_is_self_catered_ajax_request' );
function admin_accommodation_is_self_catered_ajax_request() {
	if ( isset($_REQUEST) ) {
...
add_action( 'wp_ajax_admin_accommodation_list_room_types_ajax_request', 'admin_accommodation_list_room_types_ajax_request' );
function admin_accommodation_list_room_types_ajax_request() {
...
add_action( 'wp_ajax_admin_accommodation_is_price_per_person_ajax_request', 'admin_accommodation_is_price_per_person_ajax_request' );
function admin_accommodation_is_price_per_person_ajax_request() {
	if ( isset($_REQUEST) ) {
		$nonce = wp_kses($_REQUEST['nonce'], '');
		$accommodation_id = wp_kses($_REQUEST['accommodationId'], '');
		if (wp_verify_nonce( $nonce, 'accommodation_vacancy_entry_form' )) {
...
function accommodation_vacancies_admin_display() {
	...
	if (isset($_GET['action']) && $_GET['action'] == 'delete_all_vacancies') {
		...
		$room_type_query = list_room_types();
...
class accommodation_vacancy_admin_list_table extends WP_List_Table {
	...
	*/
	...
	function prepare_items() {
		...
		$accommodation_vacancy_results = list_all_accommodation_vacancies($accommodation_id, $room_type_id, $orderby, $order, $paged, $per_page);
	...
	function handle_form_submit() {
		...
		if ((isset($_POST['insert']) || isset($_POST['update'])) && check_admin_referer('accommodation_vacancy_entry_form')) {
			...
			if (isset($_POST['insert'])) {
				...
				if (!empty($error)) {
					...
					create_accommodation_vacancy($start_date, $end_date, $accommodation_id, $room_type_id, $room_count, $price_per_day, $price_per_day_child);
				...
				if (!empty($error)) {
					...
					update_accommodation_vacancy($vacancy_id, $start_date, $end_date, $accommodation_id, $room_type_id, $room_count, $price_per_day, $price_per_day_child);
	...
	function render_entry_form() {
		...
		if (!empty($edit)) {
			$vacancy_object = get_accommodation_vacancy($edit);
		}
		...
		if (isset($_POST['room_count'])) {
			$room_count = intval(wp_kses($_POST['room_count'], '1'));
		...
		if (isset($_POST['price_per_day'])) {
			$price_per_day = floatval(wp_kses($_POST['price_per_day'], '2'));
		} else if ($vacancy_object) {
		...
		if (isset($_POST['price_per_day_child'])) {
			$price_per_day_child = floatval(wp_kses($_POST['price_per_day_child'], '2'));
		} else if ($vacancy_object) {
