...
add_action('admin_menu' , 'cruise_schedule_admin_page');

function cruise_schedule_admin_page() {
	$hook = add_submenu_page('edit.php?post_type=cruise', __('BYT Cruise schedule management', 'bookyourtravel'), __('Schedule', 'bookyourtravel'), 'edit_posts', basename(__FILE__), 'cruise_schedule_admin_display');
	add_action( "load-$hook", 'cruise_schedule_add_screen_options');
}
...
add_filter('set-screen-option', 'cruise_schedule_set_screen_options', 10, 3);
...
function cruise_schedule_admin_head() {
	...
	cruise_schedule_admin_styles();
}
add_action( 'admin_head', 'cruise_schedule_admin_head' );		

add_action( 'wp_ajax_cruise_is_price_per_person_ajax_request', 'cruise_is_price_per_person_ajax_request' );
...
add_action( 'wp_ajax_cruise_list_cabin_types_ajax_request', 'cruise_list_cabin_types_ajax_request' );
...
function cruise_schedule_admin_styles() {
	...
	echo '	.wp-list-table .column-CruiseName { width: 350px; }';
...
add_action( 'wp_ajax_cruise_type_is_repeated_ajax_request', 'cruise_type_is_repeated_ajax_request' );
	...
	*/
	...
	function get_columns() {
		return $columns= array(
			...
			'DurationDays'=>__('Duration Days', 'bookyourtravel'),
	...
	function prepare_items() {
		...
		$cruise_schedule_results = list_cruise_schedules($paged, $per_page, $orderby, $order, $day, $month, $year, $cruise_id, $cabin_type_id, $search_term);
	...
	function handle_form_submit() {
		...
		if (isset($_POST['insert']) && check_admin_referer('cruise_schedule_entry_form')) {
			...
			if (!empty($error)) {
				...
				create_cruise_schedule($cruise_id, $cabin_type_id, $cabin_count, $start_date, $duration_days, $price, $price_child, $end_date);
			...
			if (!empty($error)) {
				...
				update_cruise_schedule($schedule_id, $cruise_id, $cabin_type_id, $cabin_count, $start_date, $duration_days, $price, $price_child, $end_date);
			...
			delete_cruise_schedule($schedule_id);
	...
	function render_entry_form() {
		...
		if (!empty($edit)) {
			$schedule_object = get_cruise_schedule($edit);
		}
		...
		echo '	<td><input class="datepicker" type="text" name="end_date" id="end_date" value="' . $end_date . '" /></td>';
		echo '</tr>';
