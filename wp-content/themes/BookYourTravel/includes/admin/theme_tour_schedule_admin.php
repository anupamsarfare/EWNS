...
function tour_schedule_admin_page() {
	$hook = add_submenu_page('edit.php?post_type=tour', __('BYT Tour schedule management', 'bookyourtravel'), __('Schedule', 'bookyourtravel'), 'edit_posts', basename(__FILE__), 'tour_schedule_admin_display');
...
add_action( 'wp_ajax_tour_is_price_per_group_ajax_request', 'tour_is_price_per_group_ajax_request' );
...
add_action( 'wp_ajax_tour_type_is_repeated_ajax_request', 'tour_type_is_repeated_ajax_request' );
	...
	*/
	...
	function extra_tablenav( $which ) {
		if ( $which == "top" ){	
			...
			echo " <a class='button-primary action alignright' onclick='return confirmDelete(\"#delete_all_scheduled_entries\", \"" . __('Are you sure?', 'bookyourtravel') . "\");' href='edit.php?post_type=tour&page=theme_tour_schedule_admin.php&action=delete_all_scheduled_entries'>Delete all scheduled entries</a>";
			echo '</p>';
			echo '</div>';
		}
		if ( $which == "bottom" ){
	...
	function prepare_items() {
		...
		$tour_schedule_results = list_tour_schedules($paged, $per_page, $orderby, $order, $day, $month, $year, $tour_id, $search_term);
	...
	function handle_form_submit() {
		...
		if (isset($_POST['insert']) && check_admin_referer('tour_schedule_entry_form')) {
			...
			if (!empty($error)) {
				...
				create_tour_schedule($tour_id, $start_date, $duration_days, $price, $price_child, $max_people, $end_date);
			...
			if (!empty($error)) {
				...
				update_tour_schedule($schedule_id, $start_date, $duration_days, $tour_id, $price, $price_child, $max_people, $end_date);
	...
	function render_entry_form() {
		...
		if (!empty($edit)) {
			$schedule_object = get_tour_schedule($edit);
		}
