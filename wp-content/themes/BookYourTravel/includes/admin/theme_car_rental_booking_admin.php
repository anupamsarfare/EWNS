...
function car_rental_booking_admin_page() {
	$hook = add_submenu_page('edit.php?post_type=car_rental', __('BYT Car Rental Booking management', 'bookyourtravel'), __('Bookings', 'bookyourtravel'), 'edit_posts', basename(__FILE__), 'car_rental_bookings_admin_display');
		...
		jQuery(document).ready(function () {
			jQuery("#from_day").datepicker({
				...
				onClose: function (selectedDate) {
					...
					jQuery("#to_day").datepicker("option", "minDate", d);
				}			
			});
			jQuery("#to_day").datepicker({
				...
				onClose: function (selectedDate) {
					var d = new Date(selectedDate);
					d = new Date(d.getFullYear(), d.getMonth(), d.getDate()-1);
					jQuery("#from_day").datepicker("option", "maxDate", d);
				}
			});
		});
	';
...
class car_rental_booking_admin_list_table extends WP_List_Table {
	...
	*/
	...
	function prepare_items() {
		...
		$car_rental_booking_results = list_car_rental_bookings('', $orderby, $order, $paged, $per_page );
	...
	function handle_form_submit() {
		...
		if (isset($_POST['delete_booking'])) {
			...
			if (isset($_POST['insert']) && check_admin_referer('car_rental_booking_entry_form_nonce')) {
				...
				$booking_id = create_car_rental_booking( $first_name, $last_name, $email, $phone, $address, $town, $zip, $country, $special_requirements, $from_day, $to_day, $car_rental_id, $user_id, $total_price, $drop_off_location_id );
				...
				update_car_rental_booking($booking_id,  $first_name, $last_name, $email, $phone, $address, $town, $zip, $country, $special_requirements, $from_day, $to_day, $car_rental_id, $user_id, $total_price, $drop_off_location_id );
	...
	function render_entry_form($booking_id) {
		...
		if (!empty($edit)) {
			$booking_object = get_car_rental_booking($edit);
		}
		...
		$car_rental_results = list_car_rentals();
		...
		echo '	<td><input class="datepicker" type="text" name="from_day" id="from_day" value="' . $from_day . '" /></td>';
		echo '</tr>';
		
		echo '<tr>';
		echo '	<th scope="row" valign="top">' . __('To date', 'bookyourtravel') . '</th>';
		echo '	<td><input class="datepicker" type="text" name="to_day" id="to_day" value="' . $to_day . '" /></td>';
		echo '</tr>';
