...
add_action('admin_menu' , 'cruise_booking_admin_page');
function cruise_booking_admin_page() {
	$hook = add_submenu_page('edit.php?post_type=cruise', __('BYT Booking management', 'bookyourtravel'), __('Bookings', 'bookyourtravel'), 'edit_posts', basename(__FILE__), 'cruise_bookings_admin_display');

	add_action( "load-$hook", 'cruise_bookings_add_screen_options');
}

function cruise_bookings_set_screen_options($status, $option, $value) {
	if ( 'cruise_bookings_per_page' == $option ) 
		return $value;
}
add_filter('set-screen-option', 'cruise_bookings_set_screen_options', 10, 3);

function cruise_bookings_admin_head() {
	$page = ( isset($_GET['page'] ) ) ? esc_attr( $_GET['page'] ) : false;
	if( 'theme_cruise_schedule_booking_admin.php' != $page )
		return;

	cruise_bookings_admin_styles();
}
add_action( 'admin_head', 'cruise_bookings_admin_head' );		

function cruise_bookings_admin_styles() {
...
function cruise_bookings_add_screen_options() {
	global $wp_cruise_booking_table;
	$option = 'per_page';
	$args = array('label' => 'Bookings','default' => 50,'option' => 'cruise_bookings_per_page');
	add_screen_option( $option, $args );
 	$wp_cruise_booking_table = new cruise_booking_admin_list_table();
}

function cruise_bookings_admin_display() {
...
class cruise_booking_admin_list_table extends WP_List_Table {
	...
	*/
	...
	function column_CruiseDate($item) {
		return date("d.m.Y", strtotime($item->cruise_date));	
	}
	...
	function prepare_items() {
		...
		$cruise_bookings_results = list_cruise_bookings ( $paged, $per_page, $orderby, $order, $search_term );
	...
	function handle_form_submit() {
		...
		if (isset($_POST['delete_booking'])) {
			...
			delete_cruise_booking($booking_id);
			...
			if (isset($_POST['insert']) && check_admin_referer('cruise_booking_entry_form_nonce')) {
				...
				$booking_id = create_cruise_booking($first_name, $last_name, $email, $phone, $address, $town, $zip, $country, $special_requirements, $adults, $children, $cruise_schedule_id, $user_id, $total_price_adults, $total_price_children, $total_price, $cruise_date);
				...
				update_cruise_booking($booking_id, $first_name, $last_name, $email, $phone, $address, $town, $zip, $country, $special_requirements, $adults, $children, $cruise_schedule_id, $user_id, $total_price_adults, $total_price_children, $total_price, $cruise_date);
	...
	function render_view_form() {
		...
		if ($booking_id > 0) {
			...
			if ($booking != null) {
				...
				echo "<td>" . date("d.m.Y", strtotime($booking->cruise_date)) . "</td>";
				echo "</tr>";
				echo "<tr>";
	...
	function render_entry_form($booking_id) {
		...
		if (!empty($edit)) {
			$booking_object = get_cruise_booking($edit);
		}
		...
		if ($cruise_id > 0) {
			...
			$cruises_schedule_select = '<select id="cruise_schedule_id" name="cruise_schedule_id" onchange="cruiseBookingCruiseScheduleFilterRedirect(' . $edit . ',' . $cruise_id . ',this.value)">';
			$cruises_schedule_select .= '<option value="">' . __('Select cruise schedule', 'bookyourtravel') . '</option>';
			
			$cruises_schedule_results = list_cruise_schedules (null, 0, 'Id', 'ASC', 0, 0, 0, $cruise_id, '');
			if ( count($cruises_schedule_results) > 0 && $cruises_schedule_results['total'] > 0 ) {
				foreach ($cruises_schedule_results['results'] as $cruises_schedule_result) {
					$schedule_date = date('Y-m-d', strtotime($cruises_schedule_result->start_date));
		...
		echo '	<td><input class="datepicker" type="text" name="cruise_date" id="cruise_date" value="' . $cruise_date . '" /></td>';
		echo '</tr>';
		...
		if ($booking_object) {
			...
			if ($cruise_id > 0 && $cruise_schedule_id > 0) {
