...
function car_rentals_search_groupby( $groupby, &$wp_query ) {
	...
	if ( isset($wp_query->query_vars['post_type']) && $wp_query->query_vars['post_type'] == 'car_rental' ) {		
		...
		if ($search_only_available && (isset($date_from) || isset($date_to))) {
			...
			$groupby .= ' HAVING cars_available > cars_booked ';
...
function list_car_rentals_count ( $paged = 0, $per_page = -1, $orderby = '', $order = '', $location_id = 0, $car_types_array = array(), $search_args = array(), $featured_only = false, $author_id = null, $include_private = false ) {
	return list_car_rentals ($paged, $per_page, $orderby, $order, $location_id, $car_types_array, $search_args, $featured_only, $author_id, $include_private, true);
}
	
function list_car_rentals( $paged = 0, $per_page = -1, $orderby = '', $order = '', $location_id = 0, $car_types_array = array(), $search_args = array(), $featured_only = false, $author_id = null, $include_private = false, $count_only = false ) {
	...
	if ( isset($search_args['prices']) )
		$args['byt_prices'] = (array)$search_args['prices'];
	...
	add_filter('posts_where', 'car_rentals_search_where', 10, 2);
			
	if ($search_only_available) {
		add_filter('posts_fields', 'car_rentals_search_fields', 10, 2 );
		add_filter('posts_groupby', 'car_rentals_search_groupby', 10, 2 );
	}
	...
	remove_filter('posts_where', 'car_rentals_search_where');
		
	if ($search_only_available) {
		remove_filter('posts_fields', 'car_rentals_search_fields' );
		remove_filter('posts_groupby', 'car_rentals_search_groupby');
	}
