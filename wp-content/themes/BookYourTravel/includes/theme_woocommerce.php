...
function byt_woocommerce_init() {
	...
	add_filter( 'template_include', 'byt_woocommerce_template_include' );
	...
	if (!term_exists(BOOKYOURTRAVEL_WOO_PRODUCT_CAT_ACCOMMODATIONS, 'product_cat')) {
		byt_woocommerce_create_product_category(BOOKYOURTRAVEL_WOO_PRODUCT_CAT_ACCOMMODATIONS, __('Accommodations', 'bookyourtravel'), __('Accommodations category', 'bookyourtravel'));
	}
	if (!term_exists(BOOKYOURTRAVEL_WOO_PRODUCT_CAT_TOURS, 'product_cat')) {
		byt_woocommerce_create_product_category(BOOKYOURTRAVEL_WOO_PRODUCT_CAT_TOURS, __('Tours', 'bookyourtravel'), __('Tours Category', 'bookyourtravel'));
	}
	if (!term_exists(BOOKYOURTRAVEL_WOO_PRODUCT_CAT_CAR_RENTALS, 'product_cat')) {
		byt_woocommerce_create_product_category(BOOKYOURTRAVEL_WOO_PRODUCT_CAT_CAR_RENTALS, __('Car rentals', 'bookyourtravel'), __('Car Rentals Category', 'bookyourtravel'));
	}
	if (!term_exists(BOOKYOURTRAVEL_WOO_PRODUCT_CAT_CRUISES, 'product_cat')) {
		byt_woocommerce_create_product_category(BOOKYOURTRAVEL_WOO_PRODUCT_CAT_CRUISES, __('Cruises', 'bookyourtravel'), __('Cruises Category', 'bookyourtravel'));
	}
	
	// modify cart item name and link
	add_filter('woocommerce_cart_item_name', 'byt_woocommerce_modify_cart_product_title', 20, 3);
	add_filter('woocommerce_order_item_name', 'byt_woocommerce_modify_order_product_title', 20, 2);
	
	// prefill checkout form
	add_filter('woocommerce_checkout_get_value', 'byt_woocommerce_checkout_get_value', 20, 2);

	add_action('woocommerce_order_status_completed', 'byt_woocommerce_handle_woo_payment'); // Add reservetions to Booking System after payment has been completed.
	add_action('woocommerce_order_status_pending_to_processing', 'byt_woocommerce_handle_woo_payment'); // Add reservetions to Booking System after payment has been completed.
	add_action('woocommerce_order_status_pending_to_on-hold', 'byt_woocommerce_handle_woo_payment'); // Add reservetions to Booking System after payment has been completed.
...
function byt_woocommerce_add_to_cart_action($cart_item_key, $product_id, $quantity, $variation_id, $variation, $cart_item_data){
	global $woocommerce;
	global $wpdb;
...
function byt_woocommerce_handle_woo_cart_delete_item(){ // Delete item from database when is deleted from cart.
	global $wpdb;
...
function byt_woocommerce_handle_woo_payment($order_id) {
...
function byt_woocommerce_checkout_get_value($x, $input) {
	$value = null;
...
function byt_woocommerce_create_product_category($term_slug, $term_name, $term_description)
{
    wp_insert_term(
		$term_name,
...
function byt_woocommerce_template_include( $template ) {
...
function byt_woocommerce_create_product($product_name, $product_content, $sku_prefix, $booking_id, $booking_price, $product_category_slug){
	...
	$skuu = byt_woocommerce_random_sku($sku_prefix, 6);
	...
	byt_woocommerce_add_product_to_cart($booking_product_post_id);
	
	return $booking_product_post_id;
}

function byt_woocommerce_modify_order_product_title($product_title, $item) {
	global $order_id;
	$order = new WC_Order( $order_id );
	...
	if (isset($attributes['byt-booking-id'])) {
		...
		if ($is_accommodation) {
			$booking = get_accommodation_booking($booking_id);
			$date_from = date('Y-m-d', strtotime($booking->date_from));
			$date_to = date('Y-m-d', strtotime($booking->date_to));
			...
			$booking = get_tour_booking($booking_id);
			$tour_date = date('Y-m-d', strtotime($booking->tour_date));
			...
			$booking = get_car_rental_booking($booking_id);
			$from_day = date('Y-m-d', strtotime($booking->from_day));
			$to_day = date('Y-m-d', strtotime($booking->to_day));
			$product_title = '<strong>' . $booking->car_rental_name . '</strong>' . '<br />' . $from_day . __(' to ', 'bookyourtravel') . $to_day;			
		} else if ($is_cruise) {
			$booking = get_cruise_booking($booking_id);
			$cruise_date = date('Y-m-d', strtotime($booking->cruise_date));
...
function byt_woocommerce_modify_cart_product_title($product_title, $cart_item, $cart_item_key) {
...
function byt_woocommerce_add_product_to_cart($product_id) {
	if ( ! is_admin() ) {
		global $woocommerce;
		$found = false;
...
function byt_woocommerce_random_sku($prefix, $len = 6) {
	$str = '';
	for ($i = 0; $i < $len; $i++) {
